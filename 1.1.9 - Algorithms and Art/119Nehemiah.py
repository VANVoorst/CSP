...
#scale factors
scale_factor = float(trtl.textinput("Size","Enter a scale factor for the shapes example 2 or 5: "))
...
turtle_colors = []
while True:
    color_input = trtl.textinput("Color", "Enter a color (6 is the max number of colors allowed type 'done' to finish): ")
    if color_input.lower() == 'done':
        break
    else:
       
        try:
            test.color(color_input)
